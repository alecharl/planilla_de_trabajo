...
public class PanelReservas extends PanelImagen {
	...
	private JLabel lblAgregarReservas, lblEditarReservas, lblEliminarReservas,
			lblIdReservaAeliminar, lblHorarioEditar, lblSeniaAgregar;
	...
	private void initialize(Sistema sistema) {
		...
		lblIdReservaAeliminar = new JLabel("ID de la Reserva");
		lblIdReservaAeliminar.setForeground(Color.WHITE);
		lblIdReservaAeliminar.setFont(new Font("Impact", Font.PLAIN, 14));
		lblIdReservaAeliminar.setBounds(154, 488, 128, 22);

		this.add(lblIdReservaAeliminar);
...
//		for (Reserva r : sistema.getReservas()) {
//			if (cBoxIdReservaEditar.getSelectedItem().equals(r.getId() + ""))
//				cBoxIdClienteEditar
//						.setSelectedItem(r.getCliente().getId() + "");
...
//		}
	...
	public void cargarIdDeCanchasEnCbox() {
		...
		this.cBoxIdCanchasAgregar.setModel(new DefaultComboBoxModel<String>(
				nombres_de_canchas));
		this.cBoxIdCanchaEditar.setModel(new DefaultComboBoxModel<String>(
				nombres_de_canchas));
	}
					...
					try {
						...
						idCliente = Integer
								.parseInt((String) cBoxIdClienteEditar
										.getSelectedItem());
						idCancha = Integer.parseInt((String) cBoxIdCanchaEditar
								.getSelectedItem());
						...
						try {
							seña = Integer.parseInt((String) txfSeniaEditar
									.getText());
							idReserva = Integer
									.parseInt((String) cBoxIdReservaEditar
											.getSelectedItem());
							try {
								sistema.editarReserva(fecha, idCancha,
										idCliente, seña, idReserva);
								System.out
										.println("Aqui deberia Editar la reserva");
							} catch (Exception e) {
								mjeExcepcion.setText(e.getMessage());
							}
						} catch (NumberFormatException e) {
					...
					try {
						...
						try {
							seña = Integer.parseInt((String) txfSeniaAgregar
									.getText());
							idReserva = Integer.parseInt((String) txfIdReservaAgregar
											.getText());
					
								try {
									sistema.agregarReserva(fecha, idCancha,
											idCliente, seña, idReserva);
									mjeExcepcion.setText("La reserva fue realizada con exito");
								} catch (Exception e) {
									mjeExcepcion.setText(e.getMessage());
								}
						} catch (NumberFormatException e) {
